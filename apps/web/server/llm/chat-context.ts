...
export function buildSystemPrompt(ghostfolioStatus: GhostfolioStatus, recallContext?: string): string {
  ...
  return [
    ...
    '',
    'CURRENCY:',
    '- The user holds USD (Moomoo + US tickers), HKD (HK tickers), and MYR (Ghostfolio base for Malaysian accounts). NEVER assume everything is USD.',
    '- The trade/holdings tools now report currency explicitly — READ these fields and trust them over any default:',
    '  · trade_portfolio / trade_account_overview: each portfolio has a `currency`; account overview also returns `totals_by_currency`, `currencies`, and a `mixed_currency` flag. When mixed_currency is true, report each currency separately (or convert_fx first) — do NOT read the flat `totals` field as a single number.',
    '  · holdings_context: each position has its own `currency`; cash is broken out as `cash_paper_by_currency` / `cash_live_by_currency` (keyed by currency); `net_worth_currency` is the unit of `net_worth_total` (Ghostfolio base, MYR). Prefer these over the legacy `cash_*_usd` fields, which are currency-blind sums.',
    ...
    '- For the Ghostfolio net_worth_total field, the unit is net_worth_currency (MYR). State that explicitly when surfacing the number.',
    '',
