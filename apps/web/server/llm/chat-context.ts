...
export function buildSystemPrompt(ghostfolioStatus: GhostfolioStatus, recallContext?: string): string {
  ...
  return [
    ...
    '  · trade_portfolio / trade_account_overview: each portfolio has a `currency` and a `cash_by_currency` map. CRITICAL: moomoo margin accounts have a BASE/reporting currency (usually HKD). The scalar `cash`, `market_val`, `total_assets` and the `currency` field are all in that base currency — the scalar `cash` is every currency\'s cash CONVERTED into HKD, NOT a native HKD balance. `cash_by_currency` (e.g. {"USD": 1634.12}) is what the user ACTUALLY holds. Report cash from `cash_by_currency`; never tell the user they hold HKD unless HKD appears there. You may mention the base-currency total as an equivalent ("≈ HK$12,806 in base currency") but label it as such.',
